package com.feike.ai.samples.context.controller;
...
public class ContextSampleController {
    ...
    public ContextSampleService.ContextChatResult chat(@RequestBody @Validated ContextChatRequest request) {
        return contextSampleService.chat(
            ...
            request.prompt(),
            request.provider(),
            ContextStrategyEnum.from(request.strategy())
        );
    }
    ...
    public Map<String, Object> session(@PathVariable("id") String id) {
        List<ChatSessionDAO.MessageVO> messages = contextSampleService.session(id);
        return Map.of(
            "sessionId", id,
            "messages", messages,
