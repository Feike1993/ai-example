...
import com.feike.ai.production.session.service.SessionWriteConflictException;
...
/**
 * PostgreSQL 实现，把教学版 {@code JdbcChatSessionDAOImpl} 的三个并发缺陷逐个补上。
 *
 * <h2>一、整轮原子</h2>
 ...
 */
public class JdbcProductionChatSessionDAOImpl implements ProductionChatSessionDAO {

    private static final Logger log = LoggerFactory.getLogger(JdbcProductionChatSessionDAOImpl.class);
     ...
     */
    public JdbcProductionChatSessionDAOImpl(JdbcTemplate jdbc, TransactionTemplate tx, int maxAttempts) {
        this.jdbc = jdbc;
        this.tx = tx;
    ...
    @Override
    public void appendTurn(String tenantId, String sessionId, UUID turnId, String runId, String user, String assistant) {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                tx.executeWithoutResult(status -> insertTurn(tenant, sessionId, turnId, runId, userText, assistantText));
                return;
    ...
    public boolean clear(String tenantId, String sessionId) {
        return jdbc.update(
            "DELETE FROM prod_chat_session WHERE session_id = ? AND tenant_id = ?",
            sessionId,
            tenantId
        ) > 0;
    }
    ...
    public boolean ownedBy(String tenantId, String sessionId) {
        Integer count = jdbc.queryForObject(
            "SELECT COUNT(*) FROM prod_chat_session WHERE session_id = ? AND tenant_id = ?",
            Integer.class,
            sessionId,
            tenantId
        );
    ...
    private void insertTurn(
        ...
        int touched = jdbc.update(
            """
                INSERT INTO prod_chat_session (session_id, tenant_id) VALUES (?, ?)
                ON CONFLICT (session_id) DO UPDATE SET updated_at = NOW()
                 WHERE prod_chat_session.tenant_id = EXCLUDED.tenant_id
                """,
            sessionId,
            tenantId
        );
