 ...
 * 也就是说重连打到另一个节点时一定续不上。生产请用 {@link RedisRunEventLog}。
 */
public class InMemoryRunEventLog implements RunEventLog {
    ...
    public void begin(String runId) {
        ...
        states.put(runId, RunState.PENDING);
    }

    @Override
    public void append(String runId, StreamEvent event) {
        events.computeIfAbsent(runId, id -> new CopyOnWriteArrayList<>()).add(event);
        states.merge(runId, RunState.STREAMING, (existing, incoming) ->
            existing.terminal() ? existing : RunState.STREAMING);
    }
