...
public class GlobalModelSettingsController {
    ...
    @GetMapping public ModelSettingsVO list(HttpServletRequest http) { requireUser(http); return settings.list(); }
    @PutMapping("/providers/{id}") public void saveProvider(@PathVariable String id, @RequestBody ProviderUpsertRequest body, HttpServletRequest http) {
        ProductionPrincipal p = requireUser(http); settings.saveProvider(id, body); models.invalidate(); audit.record(p.tenantId(), p.subject(), "model.provider.save", "/api/v1/model-settings/providers/" + id, 204, null, null, null, null);
    }
    @PutMapping("/routes/{capability}") public void saveRoute(@PathVariable String capability, @RequestBody RouteUpsertRequest body, HttpServletRequest http) {
        ProductionPrincipal p = requireUser(http); settings.saveRoute(capability, body); models.invalidate(); audit.record(p.tenantId(), p.subject(), "model.route.save", "/api/v1/model-settings/routes/" + capability, 204, null, null, null, null);
    }
    private static ProductionPrincipal requireUser(HttpServletRequest http) {
