...
public enum ErrorCodeEnum {
    ...
    INPUT_DENY("input_deny", HttpStatus.UNPROCESSABLE_CONTENT, "输入命中敏感词，已拒绝处理"),
    OUTPUT_DENY("output_deny", HttpStatus.UNPROCESSABLE_CONTENT, "输出命中敏感词，已拦截"),
    CITATION_REQUIRED("citation_required", HttpStatus.UNPROCESSABLE_CONTENT, "回答缺少有效引用，已拒绝展示"),
    MEDIA_UNSUPPORTED("media_unsupported", HttpStatus.UNPROCESSABLE_CONTENT, "不支持的媒体类型"),
    MEDIA_TOO_LARGE("media_too_large", HttpStatus.UNPROCESSABLE_CONTENT, "文件超过大小上限"),
    MEDIA_TOO_MANY("media_too_many", HttpStatus.UNPROCESSABLE_CONTENT, "一次上传的文件超过件数上限"),
    MEDIA_UNREADABLE("media_unreadable", HttpStatus.UNPROCESSABLE_CONTENT, "无法读取文档内容"),
