...
/**
 * 生产链路 SSE 的唯一出口，负责把 {@link StreamEventType} 契约变成硬约束。
 * <p>
 * 它保证三件事，而这三件正是教学样例缺的：
 * <ol>
 *   <li>每条事件带单调递增的 seq，客户端能自己发现缺口；</li>
 *   <li>终态事件恰好一条且必须是最后一条，终态之后的写入直接丢弃并告警；</li>
 *   <li>所有写出的事件同时落到 {@link RunEventLog}，断线重连才有东西可回放。</li>
 * </ol>
...
public class SseStreamWriter implements AutoCloseable {
     ...
     */
    public SseStreamWriter(String runId, EventSink sink, RunEventLog eventLog, JsonMapper jsonMapper) {
        this.runId = runId;
        this.sink = sink;
    ...
    public void meta(Map<String, Object> payload) {
        ...
        emit(StreamEventType.meta, body);
    }
     ...
     */
    public void emit(StreamEventType type, Object payload) {
        if (type.terminal()) {
    ...
    public void delta(String text) {
        ...
        writeEvent(StreamEventType.delta, text);
    }
    ...
    public void done(Object payload) {
        ...
        writeRaw(StreamEventType.done, payload == null ? Map.of() : payload);
    ...
    public void error(String code, String message) {
        ...
        writeRaw(StreamEventType.error, Map.of(
    ...
    private void writeEvent(StreamEventType type, Object payload) {
        if (terminated.get()) {
    ...
    private void writeRaw(StreamEventType type, Object payload) {
