...
public class IndexController {
    ...
    /**
     * 返回样例清单，按基础闭环与进阶（第四至十二期能力 + 第十三期导航整理）分组。
     *
    ...
    public Map<String, Object> index() {
        ...
        advancedSamples.put(
            "guardrail",
            "POST /ai-example/guardrail/chat（checks + deny-words / structure）"
        );
        advancedSamples.put(
            "ragCitation",
            "POST /ai-example/rag/query（citationMode=required）"
        );
        advancedSamples.put(
            "memoryInformedRag",
            "POST /ai-example/rag/query（queryExpansion=memory_rewrite）  POST /ai-example/rag/query/compare-memory-rewrite"
        );
        advancedSamples.put(
            "ragMemoryCompare",
            "POST /ai-example/rag/query/compare-memory"
        );
