...
| 横切补丁 | Prompt 外置（A1）、同步 `TokenUsage`（A2）、RAG 空检索拒答（A3）、Agent 答案流式（A4） | （A4 当时）SSE token 累加、逐步 tool SSE → 第十期已覆盖 |
...
| 语音 / ASR–TTS | 第二期 | 工业级第九阶段已覆盖（本机演示） | 与 Agent 主线正交 | 见 [industrial-media.md](industrial-media.md)；知识库上传后台仍明确不做 |
...
| 生产 Agent 工具策略 / 逐步审计 / 步骤时间线 | 工业级 | 已覆盖（第八阶段） | 真循环要 LLM，故探针 + 默认 E2E 不打 `/agent/stream` | 见 [industrial-agent.md](industrial-agent.md) |
