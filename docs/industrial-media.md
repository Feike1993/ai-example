...
这是工业级第九阶段，不是教学新样例。教学 `/chat` 与生产 Agent 本阶段仍只走文本。第六 / 七 / 八阶段仍见 [industrial-ha.md](industrial-ha.md)、[industrial-ops.md](industrial-ops.md)、[industrial-agent.md](industrial-agent.md)。
...
| 9b 图文 | 文本仍用 `GET /chat/stream`。`POST /chat/stream` multipart（`question` + 可选 `image`）SSE 契约不变；`meta.hasImage`。有图用 Spring AI `UserMessage` + `Media`，模型 `PRODUCTION_VISION_MODEL`（默认 `qwen-vl-plus`），Provider 默认 dashscope。检索仍按文本；落库 `[图片]` 占位，不写 blob |
