import { Alert, Badge, Button, Group, Modal, MultiSelect, Paper, PasswordInput, Select, SimpleGrid, Stack, Table, Text, TextInput, Title } from '@mantine/core'
...
export function ModelSettingsPanel() {
  ...
  return <Stack gap="lg" className="model-settings-page"><div className="model-settings-heading"><Title order={1}>模型与服务设置</Title><Text c="dimmed">按能力配置默认模型与 Provider，统一管理 AI 服务供应商与模型，满足不同业务场景的需求。</Text></div>
    ...
    <Paper withBorder p="lg" className="provider-management"><Group justify="space-between" align="flex-start"><div><Title order={2}>Provider 管理</Title><Text size="sm" c="dimmed">配置和管理各个 AI 服务提供商的连接信息与可用模型。基础地址支持企业内网与自建网关；API Key 只会加密写入。</Text></div><Button className="provider-add" onClick={() => { setEditingProvider(null); setProviderOpened(true) }}>＋ 添加 Provider</Button></Group>
      <Table mt="md"><Table.Thead><Table.Tr><Table.Th>Provider</Table.Th><Table.Th>基础地址</Table.Th><Table.Th>默认模型</Table.Th><Table.Th>能力</Table.Th><Table.Th>密钥</Table.Th><Table.Th>操作</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{settings.providers.map((p) => <Table.Tr key={p.id}><Table.Td>{p.label}</Table.Td><Table.Td>{p.baseUrl}</Table.Td><Table.Td>{p.model}</Table.Td><Table.Td><Group gap={4}>{p.capabilities.map((c) => <Badge key={c} size="xs">{capabilityLabels[c] ?? c}</Badge>)}</Group></Table.Td><Table.Td>{p.keyConfigured ? '已配置' : '未配置'}</Table.Td><Table.Td><Button variant="subtle" size="compact-sm" onClick={() => { setEditingProvider(p); setProviderOpened(true) }}>编辑</Button></Table.Td></Table.Tr>)}</Table.Tbody></Table>
    </Paper>
...
function ProviderForm({ initialProvider, onSaved, onCancel }: { initialProvider: GlobalProvider | null; onSaved: () => Promise<void>; onCancel: () => void }) {
  ...
  const submit = async () => {
    ...
    try { await putGlobalProvider(id, { label, baseUrl, model, capabilities, apiKey: apiKey || undefined }); await onSaved(); setApiKey('') }
    catch (e) { setFormError(e instanceof Error ? e.message : '保存 Provider 失败') } finally { setSaving(false) }
  }
  return <Stack gap="md"><Text size="sm" c="dimmed">连接地址支持企业内网与自建网关。{editing ? 'API Key 留空将保留当前密钥；输入新值则覆盖。' : 'API Key 将加密保存，保存后不会回显。'}</Text>{formError ? <Alert color="red" title="保存失败">{formError}</Alert> : null}<SimpleGrid cols={{ base: 1, sm: 2 }}><TextInput required label="Provider ID" value={id} disabled={editing} onChange={(e) => setId(e.currentTarget.value)} placeholder="例如 internal-gateway" description={editing ? '唯一标识创建后不可修改' : undefined} /><TextInput required label="展示名称" value={label} onChange={(e) => setLabel(e.currentTarget.value)} placeholder="例如企业模型网关" /><TextInput required label="基础地址" value={baseUrl} onChange={(e) => setBaseUrl(e.currentTarget.value)} placeholder="https://llm.example.com/v1" /><TextInput required label="默认模型" value={model} onChange={(e) => setModel(e.currentTarget.value)} placeholder="例如 qwen-max" /><MultiSelect required label="能力" data={[{ value: 'chat', label: '文本对话' }, { value: 'tools', label: '工具调用' }, { value: 'vision', label: '图像理解' }, { value: 'embedding', label: '向量检索' }, { value: 'asr', label: '语音识别' }, { value: 'tts', label: '语音合成' }]} value={capabilities} onChange={setCapabilities} /><PasswordInput label={editing ? '更新 API Key（可选）' : 'API Key（只写）'} value={apiKey} onChange={(e) => setApiKey(e.currentTarget.value)} placeholder={editing ? '留空表示保留已有密钥' : '输入后将加密保存'} /></SimpleGrid><Group justify="flex-end" mt="xs"><Button variant="default" onClick={onCancel} disabled={saving}>取消</Button><Button loading={saving} disabled={!id.trim() || !label.trim() || !baseUrl.trim() || !model.trim() || capabilities.length === 0} onClick={() => void submit()}>{editing ? '保存修改' : '加密保存'}</Button></Group></Stack>
}
